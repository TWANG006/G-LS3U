...
*/
__global__
void compute_cosPhi_sinPhi_double_kernel(double *d_out_cosPhi,
										 double *d_out_sinPhi,
										 double *d_in_Phi,
										 const int iWidth,
										 const int iHeight,
...
*/
__global__
void generate_A_b_double_kernel(double *d_out_A,
								double *d_out_b,
								const uchar *d_in_imgPadded,
								...
								const int iPaddedHeight)
{
	...
	// Global Memory offset: every block actually begin with 2 overlapped pixels
	__shared__ double cos_phi_sh[BLOCK_SIZE_16][BLOCK_SIZE_16];
		...
		threadIdx.y != 0 && threadIdx.y != BLOCK_SIZE_16 - 1)
	{
		...
		for (int i = threadIdx.y - 1; i <= threadIdx.y + 1; i++)
		{
			for (int j = threadIdx.x - 1; j <= threadIdx.x + 1; j++)
			{
				double cos_phi = cos_phi_sh[i][j];
				double sin_phi = sin_phi_sh[i][j];
				double ft = static_cast<double>(img_sh[i][j]);
...
void compute_cosPhi_sinPhi(double *d_out_cosPhi,
						   double *d_out_sinPhi,
						   double *d_in_Phi,
						   const int iWidth,
						   const int iHeight,
						   ...
						   const dim3 &threads)
{
	compute_cosPhi_sinPhi_double_kernel<<<blocks, threads>>>(d_out_cosPhi,
														     d_out_sinPhi,
															 d_in_Phi,
															 iWidth,
															 iHeight,		
															 iPaddedWidth,
															 iPaddedHeight);
	getLastCudaError("compute_cosPhi_sinPhi_double_kernel launch failed!");
}
...
void get_A_b(double *d_out_A,
			 ...
			 const dim3 &threads)
{
	generate_A_b_double_kernel<<<blocks, threads>>>(d_out_A, 
											        d_out_b,
													d_in_imgPadded,
													d_in_cosphi, 
													d_in_sinphi, 
													...
													iPaddedWidth, 
													iPaddedHeight);
	getLastCudaError("generate_A_b_double_kernel launch failed!");
}
