...
%EXAMPLES   
%   z = fp_wft2a('wfr',f);
%   z = fp_wft2a('wfr',f,10,-0.5,0.1,0.5,10,-0.5,0.1,0.5);
%   z = fp_wft2a('wff',f);
%   z = fp_wft2a('wff',f,10,-0.5,0.1,0.5,10,-0.5,0.1,0.5,5);
%
%REFERENCE
...
Ff=fft2(f); 
%meshgrid in frequency domain
[yf xf]=meshgrid(-fix(nn/2):nn-fix(nn/2)-1,-fix(mm/2):mm-fix(mm/2)-1);
%to be consistent with MATLAB convention in FFT
xf=fftshift(xf);yf=fftshift(yf);
%scale for window so that norm2 of the window is 1. 
%To be consistent with our theoretical analsyis
sn2=sqrt(4*pi*sigmax*sigmay);
...
if strcmp(type,'wfr')
    ...
    for wyt=wyl:wyi:wyh
        for wxt=wxl:wxi:wxh
            %WFT basis: its spectrum
            Fg=exp(-(xf*2*pi/mm-wxt).^2/2*sigmax*sigmax ...
                -(yf*2*pi/nn-wyt).^2/2*sigmay*sigmay)*sn2;
...
elseif strcmp(type,'wff')
    ...
    for wyt=wyl:wyi:wyh
        for wxt=wxl:wxi:wxh
            ...
            ztemp=ifft2(fft2(sf).*Fg);
            %update
            z.filtered=z.filtered+ztemp(1:m,1:n); 
        end
    end
    %scale the data
